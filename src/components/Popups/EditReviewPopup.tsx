...
const EditReviewPopup = ({
  ...
  const [mutatingStates, setMutatingStates] = useState({
    isWritingRes: false,
    isTextCorrection: false,
  });
  ...
  const fetchData = async () => {
    try {
      ...
      setMutatingStates({ ...mutatingStates, isWritingRes: false });
    }
  };
  ...
  const handleCorrectReply = async () => {
    try {
      ...
      setMutatingStates({ ...mutatingStates, isTextCorrection: false });
    }
  };
      ...
      >
        ...
        <div className='flex flex-col gap-[16px] w-full'>
          ...
          <div className='flex flex-wrap items-center justify-between gap-[10px]'>
            ...
            <button
              disabled={replyText.length < 1 || mutatingStates.isTextCorrection}
              onClick={handleCorrectReply}
              className={`bg-[#EFEFF0]  text-[14px] h-[50px] leading-[19px] font-[500] border border-transparent px-[16px] cursor-pointer py-[18px] gap-[8px] flex items-center rounded-[48px] opacity-50 ${
                replyText.length > 0 && "hover:border-[#dbdbdb] !opacity-100" 
              } ${mutatingStates.isTextCorrection && "cursor-not-allowed"}`}
            >
              {mutatingStates.isTextCorrection ? (
                <Image
                  src={"/loader.gif"}
                  alt=''
                  width={100}
